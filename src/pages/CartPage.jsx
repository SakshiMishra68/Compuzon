...
const CartPageconst  = ()const  => {
  const dispatchconst  = useAppDispatch();
  const { items: cartItems, total: cartTotal }const  = useAppSelector((state)const  => state.cart);

  if (cartItems.lengthconst  === 0) {
    return (
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div className="flex items-center gap-4">
                      <div className="flex items-center border border-gray-300 rounded-md">
                        <button
                          onClick={()const  => dispatch(updateQuantity({ productId: item.id, quantity: Math.max(0, item.quantity - 1) }))}
                        ...
                        <button
                          onClick={()const  => dispatch(updateQuantity({ productId: item.id, quantity: item.quantity + 1 }))}
                          className="p-2 hover:bg-gray-100 transition-colors"
                        >
                      ...
                      <button
                        onClick={()const  => dispatch(removeFromCart(item.id))}
